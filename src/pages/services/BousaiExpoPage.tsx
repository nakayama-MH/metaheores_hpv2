...
export const BousaiExpoPage: React.FC = () => {
// ... (state and fetch)
  ...
  return (
    <main className="min-h-screen bg-white font-sans text-gray-800">
      <PageHero 
        ...
        titleJa="防災万博/こども防災万博"
      />

      <ServiceIntroBanner 
        image="/assets/services/bousai-expo/hero-thumbnail.png"
        title="メタバースやAIなどを活用し、新しい防災教育のアイデアや取り組みを発表・体験"
        ...
        <div className="container mx-auto px-4 max-w-5xl text-center space-y-10">
          <div className="flex flex-col items-center">
            <h2 className="text-lg md:text-xl lg:text-2xl font-bold leading-relaxed tracking-wider text-gray-900">
              防災を「自分ごと」に。テクノロジーで次世代と創る、新しい体験型イベ<br className="hidden md:block" />
              ント
            </h2>
          ...
          <p className="text-sm md:text-base text-gray-600 leading-[2.2] font-medium max-w-3xl mx-auto text-justify md:text-center tracking-wide">
            「防災訓練がマンネリ化している」「若い世代が参加しない」といった課題はありませんか？最新の技術やアイデア発表会を通じ、子どもから大人まで主役となって楽しく学ぶ場を提供。地域全体の意識を高め、未来を担うリーダーを育みます。
          </p>
        </div>
      </section>
      ...
      <div className="container mx-auto px-4 max-w-5xl space-y-32">
        ...
        <section>
          ...
          <p className="mb-12 text-gray-600 leading-relaxed font-medium text-sm md:text-base tracking-wide">
            学生のアイデア発表やゲーム体験を通じ、多世代が協力して学ぶイベントです。楽しみながら知識を得ることで、地域全体の防災意識を底上げします。
          </p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-8">
            {[
              { 
                title: '子どもが主役の発表会', 
                icon: '/assets/services/bousai-expo/icon-kids.png', 
                desc: '若者が自由な発想で防災案を伝え、自ら発信する力を育てます。' 
              },
              { 
                title: '遊びが学びに変わる', 
                icon: '/assets/services/bousai-expo/icon-play.png', 
                desc: '人気ゲーム空間を使い、災害時の行動を楽しみながら習得。' 
              },
              { 
                title: '世代を超えた共創の場', 
                icon: '/assets/services/bousai-expo/icon-co-creation.png', 
                desc: '学生、専門家、地域が一体となり、共に防災の未来を考える場。' 
              }
            ].map((feature, idx) => (
              <div 
                key={idx} 
        ...
        <section>
          ...
          <p className="mb-12 text-gray-600 leading-relaxed font-medium text-sm md:text-base tracking-wide">
            若者の参加を促し、自ら行動できる次世代のリーダーと地域一丸の協力体制を育みます。
          </p>

          <div className="space-y-8">
            {[
              '若年層の防災意識を高め、地域の次世代リーダーを育成',
              '自治体や企業、学校が連携した、新しい防災モデルを構築',
              '万博等での実績を活かし、注目度の高い社会貢献活動が可能'
            ].map((item, idx) => (
            ...
            {news.length > 0 ? news.map((item) => (
              <Link key={item.id} to={`/blog/${item.id}`} className="group block">
                <div className="aspect-video rounded-lg overflow-hidden bg-gray-100 mb-4 border border-gray-50 shadow-sm">
                  <img 
                    src={item.eyecatch?.url || '/assets/services/bousai-expo/hero-thumbnail.png'} 
                    alt={item.title} 
