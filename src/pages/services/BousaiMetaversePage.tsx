...
export const BousaiMetaversePage: React.FC = () => {
// ... (state and fetch)
  ...
  return (
    <main className="min-h-screen bg-white font-sans text-gray-800">
      ...
      <ServiceIntroBanner 
        image="/assets/services/bousai-metaverse/hero-thumbnail.png"
        title="災害時の避難や備えを、仮想空間上で体験しながら学べる防災教育・訓練サービス。"
        ...
        <div className="container mx-auto px-4 max-w-5xl text-center space-y-10">
          <div className="flex flex-col items-center">
            <h2 className="text-lg md:text-xl lg:text-2xl font-bold leading-relaxed tracking-wider text-gray-900">
              「もしも」の備えを「いま」体験する。日常に潜む災害リスクをいざとい<br className="hidden md:block" />
              うときに動ける没入型防災訓練。
            </h2>
          ...
          <p className="text-sm md:text-base text-gray-600 leading-[2.2] font-medium max-w-3xl mx-auto text-justify md:text-center tracking-wide">
            従来の避難訓練では再現できなかった火災の煙や浸水の恐怖を、バーチャル空間で安全に、かつリアルに体験。高い没入感により、危機意識を「自分事化」させる次世代の防災教育プラットフォームです。
          </p>
        </div>
      </section>
      ...
      <div className="container mx-auto px-4 max-w-5xl space-y-32">
        ...
        <section>
          ...
          <p className="mb-12 text-gray-600 leading-relaxed font-medium text-sm md:text-base tracking-wide">
            それぞれの地域や、国の実態に則してカスタマイズ可能で、災害を安全にシミュレーションすることができます。
          </p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-8">
            {[
              { 
                title: '災害シミュレーション', 
                icon: '/assets/services/bousai-metaverse/icon-simulation.png', 
                desc: 'リアルな炎や水の挙動により、避難の難しさを体感。' 
              },
              { 
                title: 'マルチプレイ訓練', 
                icon: '/assets/services/bousai-metaverse/icon-multiplay.png', 
                desc: '複数人で同時ログインし、協力して避難する連携訓練が可能。' 
              },
              { 
                title: '場所を選ばない実施', 
                icon: '/assets/services/bousai-metaverse/icon-anywhere.png', 
                desc: '広いスペースは不要。デバイスひとつで、どこでも訓練会場に。' 
              }
            ].map((feature, idx) => (
              <div 
                key={idx} 
        ...
        <section>
          ...
          <p className="mb-12 text-gray-600 leading-relaxed font-medium text-sm md:text-base tracking-wide">
            誰も取り残さない、楽しく実践的な防災訓練を提供します。
          </p>

          <div className="space-y-8">
            {[
              '従来の訓練では得られなかった、高い緊張感と実践的な判断力の習得',
              '訓練結果のデータ化により、防災意識の課題を客観的に分析',
              '子供から大人まで、ゲーム感覚で自発的に学べる防災意識の向上'
            ].map((item, idx) => (
            ...
            {news.length > 0 ? news.map((item) => (
              <Link key={item.id} to={`/blog/${item.id}`} className="group block">
                <div className="aspect-video rounded-lg overflow-hidden bg-gray-100 mb-4 border border-gray-50 shadow-sm">
                  <img 
                    src={item.eyecatch?.url || '/assets/services/bousai-metaverse/hero-thumbnail.png'} 
                    alt={item.title} 
