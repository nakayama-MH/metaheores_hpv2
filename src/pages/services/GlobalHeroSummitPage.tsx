...
export const GlobalHeroSummitPage: React.FC = () => {
// ... (state and fetch)
  ...
  return (
    <main className="min-h-screen bg-white font-sans text-gray-800">
      ...
      <ServiceIntroBanner 
        image="/assets/services/global-hero-summit/hero-thumbnail.png"
        title="持続可能なウェルビーイング社会を目指した、世界中の挑戦を推進する未来共創サミット。"
      ...
      <section className="bg-gray-50 py-20 md:py-32 mb-32">
        <div className="container mx-auto px-4 max-w-5xl text-center space-y-10">
          ...
          <p className="text-sm md:text-base text-gray-600 leading-[2.2] font-medium max-w-3xl mx-auto text-justify md:text-center tracking-wide">
            社会課題の解決を志すリーダー、クリエイター、企業が一堂に会する国際カンファレンス。最新事例の共有にとどまらず、新しい「共創プロジェクト」をその場で発足させることを目的とした超実践型会議です。
          </p>
        </div>
      </section>
      ...
      <div className="container mx-auto px-4 max-w-5xl space-y-32">
        ...
        <section>
          ...
          <p className="mb-12 text-gray-600 leading-relaxed font-medium text-sm md:text-base tracking-wide">
            子供たちが中心となり、人類の「価値」を証明する歴史的な舞台を目指し、次世代のリーダー（HERO）がAI、平和、教育などの分野で未来を創る共創の場を提供します。
          </p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-8">
            {[
              { 
                title: '世界基準の議論', 
                icon: '/assets/services/global-hero-summit/icon-discussion.png', 
                desc: '有識者が登壇し、XR、AI、サステナビリティの最前線を議論。' 
              },
              { 
                title: '共創セッション', 
                icon: '/assets/services/global-hero-summit/icon-session.png', 
                desc: '参加者が議論に加わり、解決策を共に考える分科会形式。' 
              },
              { 
                title: 'プロジェクト発掘', 
                icon: '/assets/services/global-hero-summit/icon-project.png', 
                desc: '生まれたアイデアに対し、資金や技術支援を行う仕組みを完備。' 
              }
            ].map((feature, idx) => (
              <div 
                key={idx} 
        ...
        <section>
          ...
          <p className="mb-12 text-gray-600 leading-relaxed font-medium text-sm md:text-base tracking-wide">
            多様な有識者や子どもたちが交流し、持続可能な社会への具体的なアクションを共創します。
          </p>

          <div className="space-y-8">
            {[
              'グローバルな最新知見と、即座に活用できる人的ネットワークの構築',
              '自社だけでは解決できない課題の異業種連携による突破口の発見',
              '社会課題解決のリーダーとしての、国際的なプレゼンスの向上'
            ].map((item, idx) => (
            ...
            {news.length > 0 ? news.map((item) => (
              <Link key={item.id} to={`/blog/${item.id}`} className="group block">
                <div className="aspect-video rounded-lg overflow-hidden bg-gray-100 mb-4 border border-gray-50 shadow-sm">
                  <img 
                    src={item.eyecatch?.url || '/assets/services/global-hero-summit/hero-thumbnail.png'} 
                    alt={item.title} 
