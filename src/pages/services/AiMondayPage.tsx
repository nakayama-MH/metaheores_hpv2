...
export const AiMondayPage: React.FC = () => {
// ... (state and fetch)
  ...
  return (
    <main className="min-h-screen bg-white font-sans text-gray-800">
      ...
      <ServiceIntroBanner 
        image="/assets/services/ai-monday/hero-thumbnail.png"
        title="最新AI技術を毎週月曜日に実践的に学び、日常の武器に変えるためのコミュニティ。"
        ...
        <div className="container mx-auto px-4 max-w-5xl text-center space-y-10">
          <div className="flex flex-col items-center">
            <h2 className="text-lg md:text-xl lg:text-2xl font-bold leading-relaxed tracking-wider text-gray-900">
              月曜日を、AIがあたりまえの未来を考える日に。AIの最新情報をアップデ<br className="hidden md:block" />
              ートするコミュニティ。
            </h2>
          ...
          <p className="text-sm md:text-base text-gray-600 leading-[2.2] font-medium max-w-3xl mx-auto text-justify md:text-center tracking-wide">
            週の始まりの月曜日に、最速でAIのトレンドをキャッチアップ。専門家から初心者までが集まり、「今週は何ができるようになったか」をウェビナー・セミナーを通じて、カジュアルに議論し、学び合うAI特化型のコミュニティです。
          </p>
        </div>
      </section>
      ...
      <div className="container mx-auto px-4 max-w-5xl space-y-32">
        ...
        <section>
          ...
          <p className="mb-12 text-gray-600 leading-relaxed font-medium text-sm md:text-base tracking-wide">
            毎週月曜日、無料で参加できる、生成AIの実践的なノウハウや最新情報を、異業種・多世代の参加者と共に学ぶコミュニティイベントです。
          </p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-8">
            {[
              { 
                title: 'ニュース解説', 
                icon: '/assets/services/ai-monday/icon-news.png', 
                desc: '前週のAI関連のアップデートを、専門家が分かりやすく解説。' 
              },
              { 
                title: '事例シェア', 
                icon: '/assets/services/ai-monday/icon-share.png', 
                desc: '参加者のAI成果物や、役立つプロンプトを公開し合う文化。' 
              },
              { 
                title: 'ネットワーキング', 
                icon: '/assets/services/ai-monday/icon-networking.png', 
                desc: 'エンジニアとビジネスサイドで、AIビジネスの種を見つける交流。' 
              }
            ].map((feature, idx) => (
              <div 
                key={idx} 
        ...
        <section>
          ...
          <p className="mb-12 text-gray-600 leading-relaxed font-medium text-sm md:text-base tracking-wide">
            生成AIを実践的に学び、日常の業務に応用することで、AIスキルを向上させます。
          </p>

          <div className="space-y-8">
            {[
              '膨大なAI情報の波に飲まれず、必要な情報だけを効率的に習得',
              '異業種のAI活用事例を知り、自社ビジネスへの応用アイデアが湧く',
              '気軽に質問できる場があることで、個人のAI活用スキルが自然と向上'
            ].map((item, idx) => (
            ...
            {news.length > 0 ? news.map((item) => (
              <Link key={item.id} to={`/blog/${item.id}`} className="group block">
                <div className="aspect-video rounded-lg overflow-hidden bg-gray-100 mb-4 border border-gray-50 shadow-sm">
                  <img 
                    src={item.eyecatch?.url || '/assets/services/ai-monday/hero-thumbnail.png'} 
                    alt={item.title} 
