...
// ... (fetchData and state logic)
  ...
  return (
    <main className="min-h-screen bg-white font-sans text-gray-800">
      <PageHero 
        titleEn="XR & AI SOLUTIONS" 
        titleJa="XRソリューション"
      />

      <ServiceIntroBanner 
        image="/assets/services/xr-solutions/hero-thumbnail.png"
        title="VR・ARを活用した課題解決。没入体験による新たなソリューションを提供。"
        ...
        <div className="container mx-auto px-4 max-w-5xl text-center space-y-10">
          <div className="flex flex-col items-center">
            <h2 className="text-lg md:text-xl lg:text-2xl font-bold leading-relaxed tracking-wider text-gray-900">
              不可能を可能にし、未来の課題を解決するテクノロジーで、社会の「不」<br className="hidden md:block" />
              を解消。メタバースを活用し、安全で効率的、感動的な社会を創造。
            </h2>
          ...
          <p className="text-sm md:text-base text-gray-600 leading-[2.2] font-medium max-w-3xl mx-auto text-justify md:text-center tracking-wide">
            従来の研修では伝えきれない、現場のリアルや危険な状況。私たちはVR（仮想現実）やMR（複合現実）を駆使し、場所や時間の制約を超えた「没入体験」を提供します。防災・教育・地域創生など、あなたの活動に最適な最新の解決策を共に創り上げます。
          </p>
        </div>
      </section>
      ...
      <div className="container mx-auto px-4 max-w-5xl space-y-32">
        ...
        <section>
          ...
          <p className="mb-12 text-gray-600 leading-relaxed font-medium text-sm md:text-base tracking-wide">
            VRやMR技術を使い、現実では難しい体験をデジタルで再現します。教育や防災などの課題に対し、最新技術を用いた効果的な解決策をオーダーメイドで提供します。
          </p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-8">
            {[
              { 
                title: '没入感ある学び', 
                icon: '/assets/services/xr-solutions/icon-immersion.png', 
                desc: '仮想空間での体験を通じて、短期間で高い学習効果を得られます。' 
              },
              { 
                title: '「自分ごと」にする', 
                icon: '/assets/services/xr-solutions/icon-ownership.png', 
                desc: '物語や体験を通じ、内容を自分の課題として深く実感させます。' 
              },
              { 
                title: '場所を再現する技術', 
                icon: '/assets/services/xr-solutions/icon-reproduction.png', 
                desc: '現実の街や施設をデジタルで再現し、観光や業務を効率化します。' 
              }
            ].map((feature, idx) => (
              <div 
                key={idx} 
              ...
              >
                <div className="w-16 h-16 md:w-24 md:h-24 mb-4 md:mb-8 flex items-center justify-center">
                  <img src={feature.icon} alt={feature.title} className="w-full h-full object-contain" onError={(e) => e.currentTarget.style.display = 'none'} />
                </div>
        ...
        <section>
          ...
          <p className="mb-12 text-gray-600 leading-relaxed font-medium text-sm md:text-base tracking-wide">
            場所や時間の制限なく、安全に本格的な疑似体験をすることで学習効果を最大化します。
          </p>

          <div className="space-y-8">
            {[
              '時間や場所の制限なく、誰でも本格的な体験学習が可能',
              '言葉では伝わりにくい「本質的な価値」を直感的に伝える',
              '社会課題の解決に繋がり、組織の信頼を大きく高める'
            ].map((item, idx) => (
      ...
      {/* Footer CTA Section - Full Width Whitish Gray Background */}
