...
export const EggJamPage: React.FC = () => {
// ... (state and fetch)
  ...
  return (
    <main className="min-h-screen bg-white font-sans text-gray-800">
      ...
      <ServiceIntroBanner 
        image="/assets/services/egg-jam/hero-thumbnail.png"
        title="子どもたちがメタバースやAI技術を活用した、社会課題解決に挑むオンラインコミュニティ"
      ...
      <section className="bg-gray-50 py-20 md:py-32 mb-32">
        <div className="container mx-auto px-4 max-w-5xl text-center space-y-10">
          ...
          <p className="text-sm md:text-base text-gray-600 leading-[2.2] font-medium max-w-3xl mx-auto text-justify md:text-center tracking-wide">
            新しいことに挑戦する人、何かを創り出したい人が集まるオンライン＆オフラインのコミュニティ。互いのアイデアを否定せず、称え合い、高め合う「心理的安全性の高い場」を提供します。
          </p>
        </div>
      </section>
      ...
      <div className="container mx-auto px-4 max-w-5xl space-y-32">
        ...
        <section>
          ...
          <p className="mb-12 text-gray-600 leading-relaxed font-medium text-sm md:text-base tracking-wide">
            最新テクノロジーに特化した、単なる教室ではなく、参加者全員が先生・生徒となり、学びと創造を共有するフラットな関係性を重視します。
          </p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-8">
            {[
              { 
                title: 'ミートアップ', 
                icon: '/assets/services/egg-jam/icon-meetup.png', 
                desc: '定期的な交流会を通じて、進捗の報告やメンバーの募集が可能。' 
              },
              { 
                title: 'メンタリング支援', 
                icon: '/assets/services/egg-jam/icon-mentoring.png', 
                desc: '各分野の専門家が参加し、技術や経営の相談にアドバイス。' 
              },
              { 
                title: 'リソースシェア', 
                icon: '/assets/services/egg-jam/icon-resource.png', 
                desc: '知識、制作ノウハウをメンバー間で共有し、開発コストを低減。' 
              }
            ].map((feature, idx) => (
              <div 
                key={idx} 
        ...
        <section>
          ...
          <p className="mb-12 text-gray-600 leading-relaxed font-medium text-sm md:text-base tracking-wide">
            自治体や企業との地方創生・DXプロジェクトを通じて新たな収益とキャリアを創出します。
          </p>

          <div className="space-y-8">
            {[
              '志を同じくする仲間と出会い、挫折せずに挑戦を続けられる環境',
              '公募を通じた、プロジェクト立ち上げ側としての活動の機会',
              'クローズドな場ならではの、鮮度の高い情報とノウハウの共有'
            ].map((item, idx) => (
            ...
            {news.length > 0 ? news.map((item) => (
              <Link key={item.id} to={`/blog/${item.id}`} className="group block">
                <div className="aspect-video rounded-lg overflow-hidden bg-gray-100 mb-4 border border-gray-50 shadow-sm">
                  <img 
                    src={item.eyecatch?.url || '/assets/services/egg-jam/hero-thumbnail.png'} 
                    alt={item.title} 
