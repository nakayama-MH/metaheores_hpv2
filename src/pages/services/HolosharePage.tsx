...
export const HolosharePage: React.FC = () => {
// ... (state and fetch logic)
  ...
  return (
    <main className="min-h-screen bg-white font-sans text-gray-800">
      ...
      <ServiceIntroBanner 
        image="/assets/services/holoshare/hero-thumbnail.png"
        title="いつでも、どこでも、目の前に実物大の製品をお披露目する体験型営業VRサービス。"
        ...
        <div className="container mx-auto px-4 max-w-5xl text-center space-y-10">
          <div className="flex flex-col items-center">
            <h2 className="text-lg md:text-xl lg:text-2xl font-bold leading-relaxed tracking-wider text-gray-900">
              「体験」が商談の成約率を変える。VRであらゆる製品を目の前に。<br className="hidden md:block" />
              製品をもっと手軽に、もっと魅力的に。
            </h2>
          ...
          <p className="text-sm md:text-base text-gray-600 leading-[2.2] font-medium max-w-3xl mx-auto text-justify md:text-center tracking-wide">
            言葉や動画では伝えきれない製品の「サイズ感」や「内部構造」を、顧客の目の前へ。商談のあらゆる無駄を削ぎ落とし、顧客の「欲しい」をその場で引き出す、BtoBセールスに特化したバーチャルコミュニケーションプラットフォームです。
          </p>
        </div>
      </section>
      ...
      <div className="container mx-auto px-4 max-w-5xl space-y-32">
        ...
        <section>
          ...
          <p className="mb-12 text-gray-600 leading-relaxed font-medium text-sm md:text-base tracking-wide">
            デバイスを持ち運べば、いつでも、どこでも、目の前に実物大の製品をお披露目することで「購入前の不安解消・購買意欲の向上」を狙えます。
          </p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-8">
            {[
              { 
                title: 'どこでもデモ可能', 
                icon: '/assets/services/holoshare/icon-demo.png', 
                desc: 'どこにいても同じ製品体験。出張費0にし、商談機会を最大化。' 
              },
              { 
                title: 'カスタマイズ提案', 
                icon: '/assets/services/holoshare/icon-customize.png', 
                desc: '要望に、VR空間で即座に対応。完成イメージをその場で確認。' 
              },
              { 
                title: '既存データの活用', 
                icon: '/assets/services/holoshare/icon-data.png', 
                desc: '既に持っているデータを、そのままVR化。横展開も可能。' 
              }
            ].map((feature, idx) => (
              <div 
                key={idx} 
        ...
        <section>
          ...
          <p className="mb-12 text-gray-600 leading-relaxed font-medium text-sm md:text-base tracking-wide">
            VRで「リアル」の没入感と説得力を再現し、コスト削減と営業効率向上を実現します。
          </p>

          <div className="space-y-8">
            {[
              '実物を見せることで、期待外れをなくす',
              '試作機輸送やサンプル展示の営業コストを下げ、営業効率を高める',
              '製品の魅力を「体験」に変えることで説得力を上げ、成約率を向上'
            ].map((item, idx) => (
            ...
            {news.length > 0 ? news.map((item) => (
              <Link key={item.id} to={`/blog/${item.id}`} className="group block">
                <div className="aspect-video rounded-lg overflow-hidden bg-gray-100 mb-4 border border-gray-50 shadow-sm">
                  <img 
                    src={item.eyecatch?.url || '/assets/services/holoshare/hero-thumbnail.png'} 
                    alt={item.title} 
